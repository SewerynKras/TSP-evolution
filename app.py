...
#from libs.garden import FigureCanvasKivyAgg
...
class EvolutionSimulator(App):
    ...
    def sstart(self):
        ...
        if self.population == []:
            ...
            self.population = Evolution._ratePopulation(
                self.population, self.target)
        Clock.schedule_interval(self.update, .1)

    def update(self, dt):
        self.population = Evolution.evolvePopulation(
            population=self.population, Target=self.target)
        ...
        self.best_label.text = "Best individual: \"{}\"".format(
            self.population[0][0])
        self.score_label.text = "Score: {}/{}".format(
            self.population[0][1], len(self.target))
        if self.population[0][1] == len(self.target):
            self.sstop()
    ...
    def plotGraph(self, best=[], aver=[], worst=[], generation=0):
        ...
        axes.plot(range(generation), aver,
                  label="Average", color="#E8FF26", lw=4)
        axes.plot(range(generation), worst,
                  label="Worst", color="#DF1F1F", lw=4)
        self.canvas.draw_idle()
